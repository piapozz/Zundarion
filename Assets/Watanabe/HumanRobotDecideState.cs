...
public class HumanRobotDecideState : BaseEnemyState
{
    private AnimatorStateInfo stateInfo;
    private EnemyHumanRobot enemyHumanRobot = null;
    ...
    public override void Enter(BaseEnemy enemy)
    {
        ...
        enemyHumanRobot = enemy.GetComponent<EnemyHumanRobot>();
        SetEnemy(enemy);
    ...
    public override void Execute(BaseEnemy enemy)
    {
        ...
        if (distance >= _ENEMY_DISTANCE_NEAR)
        {
            ...
            if (enemyHumanRobot.GetFury() >= _ENEMY_FURY_COST_JUMPATTACK)
            {
                enemy.ChangeState(new BearJumpAttackState());
                enemyHumanRobot.ChangeFury(_ENEMY_FURY_COST_JUMPATTACK);
            }
            // ä‘çáÇ¢ÇéÊÇÈ
        ...
        else if (distance <= _ENEMY_DISTANCE_NEAR)
        {
            ...
            if (enemyHumanRobot.GetFury() >= _ENEMY_FURY_COST_UPPER)
            {
                enemy.ChangeState(new BearUpperState());
                enemyHumanRobot.ChangeFury(_ENEMY_FURY_COST_UPPER);
            }
